...
{
    std::cout << QString("[%1] Socket error [code=%2]: %3")
            .arg(mSocket->peerName())
            .arg(QString::number(socketError))
            .arg(mSocket->errorString()).toStdString();
...
bool SocketConnection::enoughBytesAvailableOnSocket(int bytes) const
{
    ...
    if(!enoughBytes)
        std::cout << "Want " << bytes << " but only "<< mSocket->bytesAvailable() << " are available on the socket atm.";
    return enoughBytes;
}
...
bool SocketConnection::socketReceive(void *packPointer, int packSize) const
{
    ...
    if(r <= 0)
    {
        std::cout << "Error when receiving data from socket.";
        return false;
    }
    return true;
}
